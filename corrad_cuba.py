...
class Landsat(object):
    ...
    def __init__(self, ruta, umbral=22, hist=1000, dtm = 'plano'):
        ...
        if self.sat == 'L8':
            s = "http://earthexplorer.usgs.gov/browse/landsat_8/" + self.escena[:4] + "/" + self.escena[-7:-4] + "/" + self.escena[-3:] + "/" + usgs_id + ".jpg"
            print s
    ...
    ######VAMOS A USAR EL DTM REAL GENERADO. HAY QUE CAMABIARLE EL NO DATA POR 0 Y HAY QUE RECORTAR EL DTM Y LAS ESCENAS CON LA EXTENSION DEL WRS
    ...
    def wrs_mask(self):
        ...
        '''este metodo recorta todas las escenas de un wrs con la extension del propio wrs, de modo que cada escena tendra excatamente la misma
        extension y podra ser tratada en conjunto de cara a generar la bpa en el siguiente paso'''

        ruta_rad = os.path.join(self.rad, self.escena)
        baserep = os.path.join(r'E:\cuba\rad', self.escena)
        if not os.path.exists(baserep):
            os.makedirs(baserep)
        shape = r'C:\Cuba\data\Extent_014045_big.shp'

        for i in os.listdir(ruta_rad):
            
            if i.endswith('.img'):
                
                banda = os.path.join(ruta_rad, i)
                print banda
                bandaout = os.path.join(baserep, i)

                t = time.time()
                print "Reproyectando " + i

                cmd = ["gdalwarp", "-dstnodata" , "0", "-cutline", "-crop_to_cutline", "-tr", "30", "30", "-of", "ENVI"]
                #cmd = ["gdalwarp", "-cutline", "-crop_to_cutline", "-of", "ENVI"] #PROBAR A DEJAR EL DTM ORIGINAL -9999 A VER SI SALE MEJOR
                cmd.append(banda)
                cmd.append(bandaout)
                cmd.insert(4, shape) #seria el 4/2 con/sin el dst nodata
        ...
        for i in os.listdir(self.ruta_escena):
            ...
            if i.endswith('Fmask.img'):
                ...
                banda = os.path.join(self.ruta_escena, i)
                print banda
                bandaout = os.path.join(baserep, i)
